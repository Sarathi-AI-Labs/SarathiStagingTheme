...
if ( 'slider' === $hero_version ) :
	...
	<section class="sarathi-hero-banner sarathi-hero-slider-version<?php echo esc_attr( $section_class ); ?>" id="<?php echo esc_attr( $section_id ); ?>" aria-label="Banner Slider"<?php echo $section_style; ?>>
			...
			<div class="sarathi-slider-track">
				<?php if ( ! empty( $slides ) && is_array( $slides ) ) : ?>
					<?php foreach ( $slides as $index => $slide ) : ?>
						...
						?>
						<div class="sarathi-slide<?php echo esc_attr( $active_class ); ?>" data-index="<?php echo esc_attr( $index ); ?>">
							<?php if ( $img_src ) : ?>
								<img src="<?php echo esc_url( $img_src ); ?>" alt="<?php echo esc_attr( $caption ); ?>" class="sarathi-slide-image" loading="<?php echo 0 === $index ? 'eager' : 'lazy'; ?>">
							<?php endif; ?>
									...
									<div class="sarathi-slide-caption">
										...
										<?php if ( $btn_link ) : ?>
											<div class="sarathi-slide-actions">
												<a href="<?php echo esc_url( $btn_link['url'] ); ?>" class="sarathi-hero-btn sarathi-hero-btn-<?php echo esc_attr( $btn_style ); ?>" target="<?php echo esc_attr( ! empty( $btn_link['target'] ) ? $btn_link['target'] : '_self' ); ?>">
...
elseif ( 'inner_page' === $hero_version ) :
	...
	?>
	<section class="sarathi-hero-banner sarathi-inner-page-banner<?php echo esc_attr( $section_class ); ?>" id="<?php echo esc_attr( $section_id ); ?>" aria-label="Page Banner"<?php echo $section_style; ?>>
			...
			<div class="<?php echo esc_attr( $container_class ); ?>">
				...
				<?php if ( 'image' === $side_type && ! empty( $side_image ) ) : ?>
					<div class="sarathi-inner-banner-side-content type-image">
						<img src="<?php echo esc_url( $side_image['url'] ); ?>" alt="<?php echo esc_attr( $side_image['alt'] ); ?>" class="sarathi-inner-side-img">
					</div>
...
// =========================================================================
	...
	<section class="sarathi-hero-banner<?php echo esc_attr( $section_class ); ?>" id="<?php echo esc_attr( $section_id ); ?>" aria-label="Hero Banner"<?php echo $section_style; ?>>
		...
		<div class="sarathi-hero-background" aria-hidden="true">
			...
			<div class="sarathi-hero-floating-logo-wrap">
				<img src="<?php echo esc_url( $floating_logo_url ); ?>" alt="" class="sarathi-hero-floating-logo" loading="eager" width="900" height="900">
			</div>

			<!-- Bottom Wave & Dotted Line Decoration -->
			<div class="sarathi-hero-wave-decoration">
				<svg class="sarathi-hero-wave-svg" viewBox="0 0 1440 280" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
					<path d="M0 160C240 220 480 240 720 180C960 120 1200 140 1440 190V280H0V160Z" fill="url(#hero-wave-grad)" fill-opacity="0.08"/>
					<path d="M0 190C360 270 720 170 1080 220C1260 245 1380 230 1440 210V280H0V190Z" fill="url(#hero-wave-grad-2)" fill-opacity="0.12"/>
					<!-- Fine flowing curved lines & dotted vectors -->
					<path d="M-50 140 Q 360 260 720 120 T 1490 180" stroke="#07B6D5" stroke-opacity="0.25" stroke-width="1.5" stroke-dasharray="4 4" fill="none"/>
					<path d="M-50 170 Q 360 290 720 150 T 1490 210" stroke="#07B6D5" stroke-opacity="0.35" stroke-width="1.2" fill="none"/>
					<path d="M-50 110 Q 360 230 720 90 T 1490 150" stroke="#004b61" stroke-opacity="0.18" stroke-width="1" fill="none"/>
					<path d="M-50 200 Q 360 320 720 180 T 1490 240" stroke="#07B6D5" stroke-opacity="0.2" stroke-width="1" stroke-dasharray="2 6" fill="none"/>
					<defs>
				...
				<?php if ( ! empty( $primary_btn_url ) || ! empty( $secondary_btn_url ) ) : ?>
					<div class="sarathi-hero-actions">
						<?php if ( ! empty( $primary_btn_url ) ) : ?>
							<a href="<?php echo esc_url( $primary_btn_url ); ?>" class="sarathi-btn sarathi-hero-btn-primary" target="<?php echo esc_attr( $primary_btn_target ); ?>">
								<?php echo esc_html( $primary_btn_text ); ?>
							</a>
						<?php endif; ?>

						<?php if ( ! empty( $secondary_btn_url ) ) : ?>
							<a href="<?php echo esc_url( $secondary_btn_url ); ?>" class="sarathi-btn sarathi-hero-btn-secondary" target="<?php echo esc_attr( $secondary_btn_target ); ?>">
